;
    ...
    (function($) {
        ...
        $('#TimelineMobileCntrlCont .leftCntrl').on('click', function(e){
            var scrollBy = $('.gt-timeline').width() / 4;
            //set limits then scroll if...
            if($('.gt-timeline').position().left != -720){
                $('.gt-timeline').animate({
                    left: "-=" + scrollBy
                    }, 500, function() {
                    // Animation complete.
        ...
        $('#TimelineMobileCntrlCont .rightCntrl').on('click', function(e){
            var scrollBy = $('.gt-timeline').width() / 4;
            //set limits then scroll if...
            if($('.gt-timeline').position().left != 0){
                $('.gt-timeline').animate({
                    left: "+=" + scrollBy
                    }, 500, function() {
                    // Animation complete.
        ...
        function jqTimeLine(element, options) {
            ...
            this._offset_x = 0; // Starting position of the line
            this._current_offset_x = 0; // var used for laying out months to the hor line
            this._gap = this.options.gap; 
            this._eDotWidth = 10; // Width of the event dot shown in the ui
        ...
        jqTimeLine.prototype._generateMarkup = function() {
            var _this = this;
            var i = 0,j=0;
            var totalWidth = _this.options.numYears * this._gap;
            var containerWidth = totalWidth + 5;
        ...
        jqTimeLine.prototype._getMonthMarkup = function(num,year){
            ...
            var retStr = "";
            if(num== 0){
                retStr='<div class="horizontal-line leftend" style="left:'+_this._current_offset_x+'%">' + 
                            '<div class="year">'+year+'</div>' + 
                        '</div>';
            }else if(num%2 == 1){
                retStr = '<div class="horizontal-line month-line odd-month" style="left:'+_this._current_offset_x+'%"></div>';
            }else{
                retStr = '<div class="horizontal-line month-line even-month" style="left:'+_this._current_offset_x+'%"><div class="month">'+aMonths[num]+'</div></div>';
            }
        ...
        jqTimeLine.prototype._showToolTip=function(nLeft,strToolTip,eventId,closable){
                var _this = this;
                _this._$toolTip  = $(
                                    '<div class="e-message" id="tooltip_'+eventId+'">' +
                                        '<div class="message-pointer" style="left:'+ nLeft +'px"></div>' +
                                        strToolTip + 
                                    '</div>'
                                );
            _this.$mainContainer.after(_this._$toolTip);
        }
        ...
        jqTimeLine.prototype._getEventMarkup = function(e){
            ...
            var mn = d.getMonth();
            var totalMonths = (yn * 5) + mn;
            var leftVal = Math.ceil(_this._offset_x + totalMonths * _this.options.gap + (_this.options.gap/3)*n - _this._eDotWidth/2);
            var $retHtml = $('<div class="event" id="event_'+e.id+'" style="left:'+leftVal+'%">&nbsp;</div>').data('event',e);
                ...
                $retHtml.mouseover( 
                    function(e){
                        ...
                        for (var i = 0; i < neighborEvents.length; i++) {
                            ...
                            strToolTip = strToolTip + '<div class="title">'+ oData.title+'</div> <div class="msg" id="msg_'+oData.id+'"> '+ oData.name +'</div>';
                        };
    ...
    var $catButtons = $('.cat-item');
    var $catButtonsall = $('.cat-item-all');


    $catButtons.on('click', function(e){
        e.preventDefault();
        var $that = $(this);
        var $timelineItems = $('div.event');
        var $timelineallItems = $('cat-item-all');


        var currentSelectedCat = $that.attr('class').split(/\s+/);
        var currentSelectedCatId = currentSelectedCat[currentSelectedCat.length-1].split("-");


        console.log(currentSelectedCatId[currentSelectedCatId.length-1]);

        $timelineItems.hide();
        $timelineItems.filter(function(i){
            var thisCatId = $(this).attr('id').split("-")
            return thisCatId[thisCatId.length-1] == currentSelectedCatId[currentSelectedCatId.length-1];
        }).show("fast");

        $( ".cat-item-all" ).click(function() {
          $( "div.event" ).show( "fast");
          $( "div.event" ).show( "fast");
        });
    ...
    $('li.cat-item-all a').attr('href', '#');
    $('li.cat-item-all a').attr('class', 'cat-item');
    $('.cat-item-all').on('click', function(){
    $('li.cat-item-all').removeClass('activefilter');
    $(this).addClass('active');
});
    $('.cat-item' ).on('click', function(){
    $('.cat-item' ).removeClass('activefilter');
